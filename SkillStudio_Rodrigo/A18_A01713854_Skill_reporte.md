...
| Case | File | Entrada proporcionada | Resultado esperado | Resultado obtenido | Comentarios sobre el desempeño | Result |
|---|---|---|---|---|---|---|
| caso1 (extreme: 0 elements) | `A18_A01713854_Skill_tests/A18_A01713854_Skill_test1.txt` | `0 100` | `Invalid entry format`<br>(N = 0 viola 1 ≤ N ≤ 30) | `Invalid entry format` | Restricción violada detectada en `isValidHeader()` antes de construir el objeto `BranchBound`; `explore()` nunca se ejecuta (0 nodos), por lo que el rechazo es inmediato (O(1)). | PASS |
